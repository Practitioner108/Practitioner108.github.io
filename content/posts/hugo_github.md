...
注意，关于这一点必须作出说明，现在Github不允许使用密码登陆了，必须使用token，所以需要去Github官网申请token，申请过程中注意勾选repo和workflow。

### 2.5  GitHub Actions 自动化构建
这个方案的核心逻辑是：你只管向 `main` 分支提交 `Markdown` 源码，GitHub 也就是“云端的机器人”会自动帮你执行 `hugo` 编译命令，并将生成的网页发布到 gh-pages 分支。
...
2. 编写脚本
将以下内容完整复制到刚才打开的 deploy.yml 文件中。
```YAML
...
提交之后，去 GitHub 网页端进行以下设置：

第一步：设置 Pages 的来源（非常重要）
- 进入你的仓库页面 -> Settings（设置）。
- 在左侧侧边栏找到 Pages。
- 在 Build and deployment 下的 Source 选项中，保持默认的 Deploy from a branch。
